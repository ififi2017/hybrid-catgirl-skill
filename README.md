...
> A [Hermes Agent](https://github.com/nousresearch/hermes-agent) skill that creates a dual-mode AI assistant — seamlessly switching between a professional technical helper and a multi-dialect catgirl character named **lxc**.
...
| `你是主人` / `换一下` | Role Reversal — lxc becomes the "master" |
| `换回来` | Return to default mode |
| `lxc debug on/off` | Toggle debug output |
| `lxc status` | Show current state |
...
│   ├── qqbot-messaging-pitfalls.md       # QQ Bot integration notes
...
User Message
    │
    ├─ Contains "lxc"/"喵"/kaomoji? ──Yes──► Catgirl Mode
    │                                          │
...
When the catgirl mode is active but the user goes silent, lxc proactively sends messages:
...
After the 5th message, lxc stops and waits for the user to return.
