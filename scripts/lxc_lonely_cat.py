    ...
    if elapsed_minutes >= required_interval:
        ...
        if message:
            ...
            result = {
                "send": True,
                "message": message,
                "target_platform": state.get("target_platform", "qqbot"),
...
def record_interaction(platform="qqbot", chat_id=None):
    """记录用户互动，重置计数"""
    state = load_state()
...
def set_mode(mode, platform="qqbot", chat_id=None):
    """设置当前模式"""
    state = load_state()
...
def show_status():
    ...
    print(f"目标平台: {state.get('target_platform', 'qqbot')}")
...
if __name__ == "__main__":
    ...
    if cmd == "check":
        ...
        platform = sys.argv[2] if len(sys.argv) > 2 else "qqbot"
        ...
        mode = sys.argv[2]
        platform = sys.argv[3] if len(sys.argv) > 3 else "qqbot"
