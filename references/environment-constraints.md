...
- `send_message` - QQ bot (local only, requires config)
...
- `~/.hermes/channel_directory.json` - QQ bot channels ✓
...
**Limitations**:
- `send_message` tool may not work with qqbot in cron context
...
- See `qqbot-messaging-pitfalls.md` for messaging-specific issues
